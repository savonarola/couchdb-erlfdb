...
execute(TxObj, St, <<"GET_ESTIMATED_RANGE_SIZE">>) ->
    ...
    Value =
        case erlfdb:get_estimated_range_size(TxObj, StartKey, EndKey) of
            {erlfdb_future, _, _} = Future ->
                erlfdb:wait(Future);
            Else ->
                Else
...
execute(TxObj, St, <<"GET_KEY">>) ->
    ...
    Value =
        case erlfdb:get_key(TxObj, Selector) of
            {erlfdb_future, _, _} = Future ->
                erlfdb:wait(Future);
            Else ->
                Else
...
execute(TxObj, St, <<"SET">>) ->
    ...
    if
        not St#st.is_db -> ok;
        true -> stack_push(St, <<"RESULT_NOT_PRESENT">>)
    end,
    St;
execute(_TxObj, St, <<"LOG_STACK">>) ->
    ...
    InitAcc = {StackIdx, []},
    {_, RevItems} = lists:foldl(
        fun({Idx, Item}, {Pos, Acc}) ->
...
execute(TxObj, #st{is_directory_op = true} = St, Op) ->
    ...
    try
        ...
        _T:_R ->
            NewSt =
                case lists:member(Op, ?DIRECTORY_CREATE_OPS) of
...
execute_dir(_TxObj, St, _Dir, <<"DIRECTORY_CHANGE">>) ->
    ...
    DirIdx2 =
        case lists:nth(DirIdx1 + 1, DirList) of
            null -> ErrIdx;
            _ -> DirIdx1
        end,
    St#st{
